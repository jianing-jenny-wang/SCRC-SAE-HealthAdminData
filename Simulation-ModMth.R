...
#### Choose the Scenario ####
scenario <- 2.1 # extended Mth model starts with 2.
nDatasets_val <- 3
...
}else{path_to_input <-  paste(".../Simulation/GenDt_Mth/Simu",scenario,"_J", nDatasets_val, "_", DetectProb_scenario, Vary_Extreme_scenario, "PDetect/", sep = "")}

if(Remove_Smallest_List == "no"){
  path_to_output <- paste(".../Simulation/Simu_MCMC_Out/Mth/Simu",scenario,"_J", nDatasets_val, "_", DetectProb_scenario, "PDetect/", sep = "")
}
if(Remove_Smallest_List == "yes"){
  path_to_output <- paste(".../Simulation/Simu_MCMC_Out/Mth/Simu",scenario,"_J", nDatasets_val, "_", DetectProb_scenario, "PDetect", "woSmallestJ/", sep = "")
}
...
########### FINISH REMOVING THE SMALLEST LIST ################
...
Prev <- N_target/P
## Compute effective list effects in real generated data for posterior check
true_pdetect_bylist <- PDetect_val
true_logitlisteffect <- dtBasicSetUp_one_simu$list_effects
...
# can go on working:
nbcores <- detectCores() - 1 # 28 cores or as.numeric(Sys.getenv("NSLOTS")) 
...
workflow <- function(seed, config_param){
  ...
  Mod_GenMth_ICAR_Code <- nimbleCode( {
    ############
    ## Priors ##
    ############
  ...
  # function in modelfit_functions.R, write here because this has been wrapped into a function
  ## Create initial values for loading matrix ##
  ...
  ## Build model ##
  Mod_GenMth_ICAR <- nimbleModel(code = Mod_GenMth_ICAR_Code, name = "Mod_GenMth_ICAR",
                                 constants = config_param$Constants,
                                 data = config_param$Data, inits = Mod_GenMth_ICAR_Inits,
                                 calculate = FALSE # disable the calculation of all deterministic nodes and log-likelihood
  )
  ## Compile the nimble model ##
  cMod_GenMth_ICAR <- compileNimble(Mod_GenMth_ICAR)
  ## (1) Configuration
  Mod_GenMth_ICARConf <- configureMCMC(Mod_GenMth_ICAR, 
  ...
  Mod_GenMth_ICARMCMC <- buildMCMC(Mod_GenMth_ICARConf)
  ## (3) Compile C++ after configuration
  C_mcmc_Mod_GenMth_ICAR <- compileNimble(Mod_GenMth_ICARMCMC, project = Mod_GenMth_ICAR) 
  ## (4) Run MCMC sampling
  samples_Mod_GenMth_ICAR <- runMCMC(C_mcmc_Mod_GenMth_ICAR, 
...
## Constants 
Mod_GenMth_ICAR_Consts <- list(M = aug_size,
                               J = J,
                               K = K,
                               ...
                               sigma_loading = 10,  # standard deviation of single loading element
...
Mod_GenMth_ICAR_config <- list(nchains = nchains,
                               niter = niter,
                               nburnin = nburnin,
                               nthin = nthin,
...
output <- parLapply(cl = my_cluster, 
                    X = 9, 
                    fun = workflow, 
                    config_param = Mod_GenMth_ICAR_config ## Configuration, Data and Constants ##
)
...
## length(output) = 1
samples_Mod_GenMth_ICAR <- output[[1]] # length = 1
...
chain1 <- samples_Mod_GenMth_ICAR$samples$chain1
chain2 <- samples_Mod_GenMth_ICAR$samples$chain2
chain3 <- samples_Mod_GenMth_ICAR$samples$chain3
...
#################################################
## Variance parameter of spatial random effect ##
#################################################
...
###########################
samp1_loading_mat <- save_loading_mat(samples = chain1, names_in_chain = names_chain1) # Dim = nsamples * J
samp2_loading_mat <- save_loading_mat(samples = chain2, names_in_chain = names_chain2)
samp3_loading_mat <- save_loading_mat(samples = chain3, names_in_chain = names_chain3)

## Combine chains
samp_combo_loading_mat <- data.frame(rbind(samp1_loading_mat, samp2_loading_mat, samp3_loading_mat)) # (nchains * nsamp) * J
colnames(samp_combo_loading_mat) <- paste("loading_factor_j", 1:J, "_", D, sep = "")
...
## Baseline overall pseudo prevalence
chain_index <- rep(1:nchains,each = nsamples)
...
## Loading factor matrix Alpha 
chain_index <- rep(1:nchains,each = nsamples)
combodt_samp_loading_mat <- data.frame(Chains = chain_index,
                                       Combo_samp_loading_mat = samp_combo_loading_mat)
names_loading <- expand_grid(J = 1:J, D = 1:D)
colnames(combodt_samp_loading_mat) <- c("Chains",paste("loading_alpha_", "J", names_loading$J, "_D", names_loading$D,sep = "")) 

## Extract the WAIC
mcmc.out.WAIC <- samples_Mod_GenMth_ICAR$WAIC

## Extract all chains summary
mcmc.out.summary <- samples_Mod_GenMth_ICAR$summary$all.chains
...
TrueVals <- list(Parallel_Run = parallel_run,
                 ...
                 N_target = N_target,
                 Prev = Prev,
                 Eff_list_effects = true_logitlisteffect, # effective list effect in really the generated data
                 loading_mat = dtBasicSetUp_one_simu$alpha,
                 Basline_prev = unique(invlogit(dtBasicSetUp_one_simu$beta0)),
                 Sigma2.phi = dtBasicSetUp_one_simu$tau2,
...
NIMBLE_Setup <- list(case = case,
                     ...
                     nthin = nthin,
                     nsamples = nsamples,
                     sd_spRE_prior = Mod_GenMth_ICAR_Consts$sd_spRE_prior, # prior value for spatial RE half-cauchy variance
                     sigma_mu = Mod_GenMth_ICAR_Consts$sigma_mu, # prior value for list effect variance,
                     mu_theta = Mod_GenMth_ICAR_Consts$mu_theta, # mean of latent factors theta
                     sd_theta_prior = Mod_GenMth_ICAR_Consts$sd_theta_prior, # standard deviation of half-cauchy prior distribution for theta variance parameter
                     sigma_loading = Mod_GenMth_ICAR_Consts$sigma_loading  # standard deviation of single loading element
)

## Save data
saveRDS(samples_Mod_GenMth_ICAR$samples, file = paste0(path_to_output, "mcmc.out_ModMthICAR_samples", "_simu", task_id, ".Rdata"))
saveRDS(combodt_samp_N, file = paste0(path_to_output, "mcmc.out_ModMthICAR_N", "_simu", task_id, ".Rdata"))
saveRDS(combodt_samp_prev, file = paste0(path_to_output, "mcmc.out_ModMthICAR_prev", "_simu", task_id, ".Rdata"))
saveRDS(combodt_samp_sigma2.phi, file = paste0(path_to_output, "mcmc.out_ModMthICAR_sigma2.phi", "_simu", task_id, ".Rdata"))
saveRDS(combodt_samp_effbaselineprev, file = paste0(path_to_output, "mcmc.out_ModMthICAR_effbaselineprev", "_simu", task_id, ".Rdata"))
saveRDS(combodt_samp_resPhi, file = paste0(path_to_output, "mcmc.out_ModMthICAR_resPhi", "_simu", task_id, ".Rdata"))
saveRDS(combodt_samp_loading_mat, file = paste0(path_to_output, "mcmc.out_ModMthICAR_loading_mat", "_simu", task_id, ".Rdata"))
saveRDS(mcmc.out.WAIC, file = paste0(path_to_output, "mcmc.out_ModMthICAR_WAIC", "_simu", task_id, ".Rdata"))
saveRDS(mcmc.out.summary, file = paste0(path_to_output, "mcmc.out_ModMthICAR_summary", "_simu", task_id, ".Rdata"))

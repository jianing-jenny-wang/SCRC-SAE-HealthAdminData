---
title: "Simulation-PostprocessMt"
author: "Jianing Wang"
date: "2023-04-29"
...
```{r Read true values, include = FALSE}
...
## All the following ones use the designed values, rather than the empirical values.
  True_list_effects_simu_i <- TrueVals_simu_i$Theoretical_list_effects 
...
## Computed empirical detection probabilities across simulations
...
### Empirical detection probabilities by lists
...
### Empirical general population size (P) by areas
...
```{r Number of simulations that run through, echo = FALSE}
nsim
```
...
```{r Empirical true detection probability across simulation, echo = FALSE}
...
# The empirical mean/sd detection probabilities by list computed from the yfull data is:
...
```{r Empirical true list effects across simulation, echo = FALSE}
...
```{r GR Diag,  echo = FALSE}
samples_Mod <- list.files(path = path_to_input, pattern = paste('mcmc.out_',models, 'mcmc.out_ModMtICAR_samples_(simu\\d+).Rdata$', sep = ""))
...
ICAR_ggmcmc %>% filter(Parameter == c("N", paste0("pesudo_logit_prev[", col_index, "]"), "sigma2.phi", "beta0", paste0("logit_list_effects[", 1:J, "]"))) %>% 
...
```{r Bias TotalNtarget,  echo = FALSE}
samp_TotalNtarget <- list.files(path = path_to_input, pattern = 'mcmc.out_ModMtICAR_N_(simu\\d+).Rdata$')
...
samp_prev <- list.files(path = path_to_input, pattern = 'mcmc.out_ModMtICAR_prev_(simu\\d+).Rdata$') 
...
We plot out the rank of the K areas and see which estimated areas got off from the true values in terms of the rank. If the estimated rank matches the rank in the true values, then the point should be on the diagonal line, otherwise it will be off the diagonal line.

Each area is plotted with the size corresponds to the general population size assigned in simulations. The color shows the mean of the empirical number of observed counts across simulations, the darker (blue) the color, the larger the mean of the number of observations was for the area.
...
### Map of the Mean Area-Wise Prevalence

We only compare the mean of the area-specific prevalence. Median comparison does not make sense in this setting because each simulation has a unique true value for area $k^{th}$ prevalence, the same median doesn't mean the same prevalence (e.g. c(0.05,0.07,0,15) vs c(0.05,0.05,0.05) are the same median but different results).

```{r Map of prev, echo = FALSE,  fig.show="hold", out.width="100%"}
...
Again, note that the reason why the median is not going to be used in this visualization is that the equation in the step 2 does hold for the median statistics.
...
samp_resPhi <- list.files(path = path_to_input, pattern = 'mcmc.out_ModMtICAR_resPhi_(simu\\d+).Rdata$') 
...
samp_Sigma2.spRE<- list.files(path = path_to_input, pattern = 'mcmc.out_ModMtICAR_sigma2.phi_(simu\\d+).Rdata$')
...
samp_logitlisteffect <- list.files(path = path_to_input, pattern = 'mcmc.out_ModMtICAR_logitlisteffect_(simu\\d+).Rdata$') 
...
samp_EffBaselinePrev<- list.files(path = path_to_input, pattern = 'mcmc.out_ModMtICAR_effbaselineprev_(simu\\d+).Rdata$')
...
samp_WAIC <- list.files(path = path_to_input, pattern = 'mcmc.out_ModMtICAR_WAIC_(simu\\d+).Rdata$')
